...
> **PptxGenJS 기본 레이아웃은 10" x 5.625" (`LAYOUT_16x9`)이다.**
...
| `LAYOUT_16x10` | 10" x 6.25" | **호환 안 됨** |
| `LAYOUT_4x3` | 10" x 7.5" | **호환 안 됨** |
...
// 주의: pptx.layout = 'LAYOUT_WIDE' 설정이 선행되어야 함 (위 "필수 초기화" 참조)
const SAFE = {
  x: 0.6,        // 좌측 여백
  y: 0.5,        // 상단 여백
...
슬라이드: `LAYOUT_WIDE` 기준 **13.33" × 7.5"** (반드시 `pptx.layout = 'LAYOUT_WIDE'` 설정 필수 — 상단 "필수 초기화" 참조)
...
**★★★★ 레이아웃 초기화 검사 (최최우선 — 이것이 없으면 모든 것이 깨짐)**
- [ ] `pptx.layout = 'LAYOUT_WIDE'` 설정 확인 (PptxGenJS 인스턴스 생성 직후)
- [ ] `LAYOUT_16x9`(기본값, 10"x5.625")가 아닌 `LAYOUT_WIDE`(13.33"x7.5") 사용 확인
